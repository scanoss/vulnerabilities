...
func RunServer(config *myconfig.ServerConfig, ctx context.Context, grpcPort, httpPort string,
	...
	srv, mux, grpcGateway, opts, err := gw.SetupGateway(grpcPort, httpPort, config.TLS.CertFile, "localhost",
	...
	go func() {
		...
		defer cancel()
		if gwErr := pb.RegisterVulnerabilitiesHandlerFromEndpoint(ctx2, mux, grpcGateway, opts); gwErr != nil {
			zlog.S.Panicf("Failed to start HTTP gateway %v", gwErr)
		}
